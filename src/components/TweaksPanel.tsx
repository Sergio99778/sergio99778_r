...
// Port of v2/tweaks-panel.jsx — verbatim behavior, TypeScript types added.
//
...
export function TweaksPanel({
  ...
  const [railEnabled, setRailEnabled] = useState(
    () =>
      hasDeckStage &&
      !!(document.querySelector('deck-stage') as Element & { _railEnabled?: boolean })
        ?._railEnabled,
  );
  useEffect(() => {
...
export function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const labelLen = (o: RadioOption) =>
    String(typeof o === 'object' ? o.label : o).length;
  const maxLen = options.reduce<number>((m, o) => Math.max(m, labelLen(o)), 0);
  const segFit = { 2: 16, 3: 10 } as Record<number, number | undefined>;
  const fitsAsSegments = maxLen <= (segFit[options.length] ?? 0);
  if (!fitsAsSegments) {
    // <select> emits strings — map back to the original option value so the
    // fallback stays type-preserving (numbers, booleans) like the segment path.
    const resolve = (s: string): string | number => {
  ...
  const segAt = (clientX: number): string | number => {
    // trackRef.current is always set here — called only from pointer handlers on a mounted element
